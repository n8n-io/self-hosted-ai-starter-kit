            ...
            for spot_id in $spot_requests; do
                if [ -n "$spot_id" ] && [ "$spot_id" != "None" ]; then
                    if [ "$DRY_RUN" = true ]; then
                        ...
                        if aws ec2 cancel-spot-instance-requests --spot-instance-request-ids "$spot_id" --region "$AWS_REGION" >/dev/null 2>&1; then
...
cleanup_single_efs() {
        ...
        for mt_id in $mount_targets; do
            if [ -n "$mt_id" ] && [ "$mt_id" != "None" ]; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    if aws efs delete-mount-target --mount-target-id "$mt_id" --region "$AWS_REGION" >/dev/null 2>&1; then
        ...
        for ap_id in $access_points; do
            if [ -n "$ap_id" ] && [ "$ap_id" != "None" ]; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    if aws efs delete-access-point --access-point-id "$ap_id" --region "$AWS_REGION" >/dev/null 2>&1; then
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        if aws efs delete-file-system --file-system-id "$efs_id" --region "$AWS_REGION" >/dev/null 2>&1; then
...
cleanup_load_balancers() {
    ...
    local alb_list
    alb_list=$(aws elbv2 describe-load-balancers --output json --region "$AWS_REGION" 2>/dev/null || echo '{"LoadBalancers":[]}')
        ...
        if [ -n "$alb_arns" ] && [ "$alb_arns" != "None" ] && [ "$alb_arns" != "null" ]; then
            for alb_arn in $alb_arns; do
                if [ -n "$alb_arn" ] && [ "$alb_arn" != "None" ] && [ "$alb_arn" != "null" ]; then
                    ...
                    local tg_list
                    tg_list=$(aws elbv2 describe-target-groups --load-balancer-arn "$alb_arn" --output json --region "$AWS_REGION" 2>/dev/null || echo '{"TargetGroups":[]}')
                        ...
                        for tg_arn in $tg_arns; do
                            if [ -n "$tg_arn" ] && [ "$tg_arn" != "null" ]; then
                                if [ "$DRY_RUN" = true ]; then
                                    ...
                                    if aws elbv2 delete-target-group --target-group-arn "$tg_arn" --region "$AWS_REGION" >/dev/null 2>&1; then
                    ...
                    if [ "$DRY_RUN" = true ]; then
                        ...
                        if aws elbv2 delete-load-balancer --load-balancer-arn "$alb_arn" --region "$AWS_REGION" >/dev/null 2>&1; then
...
cleanup_cloudfront_distributions() {
    ...
    local distributions
    distributions=$(aws cloudfront list-distributions --output json --region "$AWS_REGION" 2>/dev/null || echo '{"DistributionList":{"Items":[]}}')
            ...
            for dist_id in $dist_ids; do
                if [ -n "$dist_id" ] && [ "$dist_id" != "null" ] && [ "$dist_id" != "None" ]; then
                    if [ "$DRY_RUN" = true ]; then
                        ...
                        local config
                        config=$(aws cloudfront get-distribution-config --id "$dist_id" --query DistributionConfig --output json --region "$AWS_REGION" 2>/dev/null) || continue
                        local etag
                        etag=$(aws cloudfront get-distribution-config --id "$dist_id" --query ETag --output text --region "$AWS_REGION" 2>/dev/null) || continue
                        ...
                        if [ -n "$config" ] && [ -n "$etag" ] && [ "$config" != "null" ] && [ "$etag" != "null" ]; then
                            ...
                            if aws cloudfront update-distribution \
                                ...
                                --if-match "$etag" --region "$AWS_REGION" >/dev/null 2>&1; then
        ...
        for alarm_name in $alarm_names; do
            if [ -n "$alarm_name" ]; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    if aws cloudwatch delete-alarms --alarm-names "$alarm_name" --region "$AWS_REGION" >/dev/null 2>&1; then
        ...
        for log_group in $log_groups; do
            if [ -n "$log_group" ]; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    if aws logs delete-log-group --log-group-name "$log_group" --region "$AWS_REGION" >/dev/null 2>&1; then
        ...
        for volume_id in $volume_ids; do
            if [ -n "$volume_id" ] && [ "$volume_id" != "None" ]; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    if aws ec2 delete-volume --volume-id "$volume_id" --region "$AWS_REGION" >/dev/null 2>&1; then
        ...
        for snapshot_id in $snapshot_ids; do
            if [ -n "$snapshot_id" ] && [ "$snapshot_id" != "None" ]; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    if aws ec2 delete-snapshot --snapshot-id "$snapshot_id" --region "$AWS_REGION" >/dev/null 2>&1; then
